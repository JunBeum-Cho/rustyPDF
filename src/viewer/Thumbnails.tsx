...
import { documentStore, setDocumentStore, type PageMeta } from "../state/document";
...
// Width budgeted for the actual thumbnail canvas inside the sidebar — the
// remainder is padding + space for the resize handle / label.
const HORIZONTAL_PADDING = 32;
// Minimum visual size for the canvas regardless of how narrow the sidebar gets.
...
function ThumbnailCanvas(props: ThumbnailCanvasProps) {
  ...
  createEffect(() => {
    ...
    (async () => {
      try {
        const buf = await renderPage(props.docId, props.index, currentScale, props.rotation);
        if (!cancelled) {
          paintRgba(canvas, buf);
        }
      } catch (error) {
        if (!cancelled) {
          console.error("render thumbnail failed", props.index, error);
        }
      }
    })();
    onCleanup(() => {
        ...
        <For each={doc().pages}>
          {(page, index) => (
            <button
              ...
              classList={{ active: documentStore.currentPage === index() }}
              onClick={() => setDocumentStore("currentPage", index())}
            >
              <ThumbnailCanvas
                page={page}
                index={index()}
